...
export function HospitalCreatePage() {
  ...
  const { createHospital } = useHospitals({ limit: 1 });
    ...
    return (
      <div className="flex h-screen w-screen items-center justify-center bg-gray-50 dark:bg-zinc-950">
        <div className="text-center space-y-3">
          ...
          <p className="text-xs text-zinc-500">Only Administrators can register new clinical placements.</p>
          <button
  ...
  const handleFormSubmit = async (values: any) => {
    try {
      ...
      setError(err?.message || 'Failed to register clinical hospital profile.');
    } finally {
      setLoading(false);
    }
  };
        ...
        <header className="h-16 shrink-0 border-b border-zinc-100 bg-white dark:border-zinc-800 dark:bg-zinc-950 flex items-center gap-3 px-6 z-10">
          ...
          <div>
            <div className="flex items-center gap-1.5 text-xs font-bold text-zinc-400">
              ...
              <span>/</span>
              <span className="text-zinc-500 dark:text-zinc-300">New Registry</span>
            </div>
            <h1 className="text-sm font-black text-zinc-900 dark:text-zinc-50 leading-tight">Register Hospital Placement</h1>
          </div>
        </header>
