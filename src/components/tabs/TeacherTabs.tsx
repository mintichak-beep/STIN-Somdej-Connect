...
import { 
  Users, MapPin, Bed, Bus, Zap, FileText, Megaphone, BarChart3, Settings, 
  Plus, Check, X, FileDown, Upload, Trash2, ArrowUpRight, Search, ShieldCheck,
  Edit2, Calendar, Trash, ClipboardList, Droplets, Eye, Home
} from 'lucide-react';
import { getDoc, getDocs, collection, addDoc, updateDoc } from 'firebase/firestore';
import { mockDB } from '../../services/mockData';
import { useTrainingGroups } from '../../hooks/useTrainingGroups';
import { Building, Room, RoomAssignment, Student, Vehicle, Driver, TransportSchedule, TransportAssignment, Bill, Payment } from '../../types/db';
...
export function TeacherTabs({ activeTab }: TeacherTabsProps) {
  ...
  const { trainingGroups: groups, createGroup, updateGroup, deleteGroup } = useTrainingGroups();

  const [sites, setSites] = useState<any[]>(() => {
    const saved = localStorage.getItem('stin_training_sites');
    return saved ? JSON.parse(saved) : [
      { id: '1', name: 'Somdej Hospital', location: 'Chonburi', capacity: 45, occupied: 27, status: 'Active' },
      { id: '2', name: 'Siriraj Hospital', location: 'Bangkok', capacity: 30, occupied: 8, status: 'Active' },
      { id: '3', name: 'Queen Savang Vadhana Memorial', location: 'Chonburi', capacity: 25, occupied: 0, status: 'Maintenance' },
    ];
  });
  ...
  const [dorms, setDorms] = useState<any[]>(() => {
    ...
    return saved ? JSON.parse(saved) : [
      { id: '1', building: 'Building A (Female)', room: '101', capacity: 4, occupied: 3, beds: ['Available', 'Occupied', 'Occupied', 'Occupied'] },
      { id: '2', building: 'Building A (Female)', room: '102', capacity: 4, occupied: 4, beds: ['Occupied', 'Occupied', 'Occupied', 'Occupied'] },
      { id: '3', building: 'Building B (Male)', room: '201', capacity: 4, occupied: 1, beds: ['Occupied', 'Available', 'Available', 'Available'] },
      { id: '4', building: 'Building B (Male)', room: '202', capacity: 4, occupied: 0, beds: ['Available', 'Available', 'Available', 'Available'] },
    ];
  });
  ...
  const [vehicles, setVehicles] = useState<any[]>(() => {
    ...
    return saved ? JSON.parse(saved) : [
      { id: '1', plate: 'กข-1234 ชลบุรี', model: 'Toyota Commuter Van', capacity: 12, driver: 'Sompon Deeprasert', phone: '081-555-1234', route: 'Campus -> Somdej Hospital' },
      { id: '2', plate: 'มง-5678 กรุงเทพ', model: 'Isuzu Minibus', capacity: 24, driver: 'Prasert Ngamdee', phone: '082-444-5678', route: 'Campus -> Siriraj Hospital' },
    ];
  });
  ...
  const [bills, setBills] = useState<any[]>(() => {
    ...
    return saved ? JSON.parse(saved) : [
      { id: '101', room: '101', month: 'June', year: '2026', waterUnit: 14, electricUnit: 156, total: 642, status: 'Pending', slip: 'https://images.unsplash.com/photo-1554415707-6e8cfc93fe23?auto=format&fit=crop&q=80&w=300' },
      { id: '102', room: '102', month: 'June', year: '2026', waterUnit: 22, electricUnit: 240, total: 980, status: 'Paid', slip: 'https://images.unsplash.com/photo-1554415707-6e8cfc93fe23?auto=format&fit=crop&q=80&w=300' },
      { id: '201', room: '201', month: 'June', year: '2026', waterUnit: 8, electricUnit: 98, total: 420, status: 'Unpaid' },
    ];
  });
  ...
  const [documents, setDocuments] = useState<any[]>(() => {
    ...
    return saved ? JSON.parse(saved) : [
      { id: '1', name: 'Clinical Placement Handbook 2026.pdf', size: '2.4 MB', date: '12 July 2026', category: 'Handbooks' },
      { id: '2', name: 'Water & Electricity Rate Regulations.pdf', size: '1.1 MB', date: '01 June 2026', category: 'Lodging Rules' },
      { id: '3', name: 'Dormitory Check-out Form.docx', size: '420 KB', date: '05 July 2026', category: 'Forms' },
    ];
  });
  ...
  const [announcements, setAnnouncements] = useState<any[]>(() => {
    ...
    return saved ? JSON.parse(saved) : [
      { id: '1', title: 'Urgent: Water system maintenance in Building A', content: 'There will be a scheduled water shutdown for maintenance in Building A on July 20th from 09:00 to 12:00. Please prepare accordingly.', date: '18 July 2026', priority: 'Urgent', author: 'Ajarn Somsri' },
      { id: '2', title: 'Clinical Placement guidelines for Semester 1/2026', content: 'All nursing students enrolled in Semester 1 placements are requested to download the updated evaluation logbook from the documents tab.', date: '15 July 2026', priority: 'General', author: 'Ajarn Somchai' },
    ];
  });
  ...
  const [dbBuildings, setDbBuildings] = useState<Building[]>(() => mockDB.getBuildings());
  const [dbRooms, setDbRooms] = useState<Room[]>(() => mockDB.getRooms());
  const [dbAssignments, setDbAssignments] = useState<RoomAssignment[]>(() => mockDB.getRoomAssignments());
  const [dbStudents, setDbStudents] = useState<Student[]>(() => mockDB.getStudents());
  const [dbVehicles, setDbVehicles] = useState<Vehicle[]>(() => mockDB.getVehicles());
  const [dbDrivers, setDbDrivers] = useState<Driver[]>(() => mockDB.getDrivers());
  const [dbTransportSchedules, setDbTransportSchedules] = useState<TransportSchedule[]>(() => mockDB.getTransportSchedules());
  const [dbTransportAssignments, setDbTransportAssignments] = useState<TransportAssignment[]>(() => mockDB.getTransportAssignments());
  const [dbBills, setDbBills] = useState<Bill[]>(() => mockDB.getBills());
  const [dbPayments, setDbPayments] = useState<Payment[]>(() => mockDB.getPayments());
  ...
  const [electricAmountOverride, setElectricAmountOverride] = useState<string>('');
  ...
  const [utilitiesSubTab, setUtilitiesSubTab] = useState<'record' | 'rooms' | 'ledger'>('record');
  ...
  const [assignRoomSelectedStudentId, setAssignRoomSelectedStudentId] = useState<string>('');
  ...
  const handleAssignRoom = (studentId: string, roomId: string) => {
    ...
    if (student) {
      ...
      window.dispatchEvent(new CustomEvent('cpatms_db_update', { detail: { key: 'cpatms_students' } }));
  ...
  const handleUnassignRoom = (studentId: string) => {
    ...
    if (student) {
      ...
      if (oldRoomId) {
        ...
        if (rIndex !== -1) {
          rooms[rIndex].occupiedCount = Math.max(0, (rooms[rIndex].occupiedCount || 1) - 1);
          mockDB.saveRooms(rooms);
        }
      }
  ...
  const handleGenerateBills = (e: React.FormEvent) => {
    ...
    const calculatedWaterCost = waterAmountOverride ? parseFloat(waterAmountOverride) || 0 : calcWaterUnits * waterRate;

    const calcElectricUnits = Math.max(0, currElectricMeter - prevElectricMeter);
    const calculatedElectricCost = electricAmountOverride ? parseFloat(electricAmountOverride) || 0 : calcElectricUnits * electricRate;

    const totalRoomBill = calculatedWaterCost + calculatedElectricCost + otherCharges;
    ...
    roomOccupants.forEach(student => {
      ...
      const newBill: Bill = {
        ...
        month: selectedMonth,
        year: selectedYear,
        waterUnit: parseFloat((calcWaterUnits / roomOccupants.length).toFixed(1)),
        electricUnit: parseFloat((calcElectricUnits / roomOccupants.length).toFixed(1)),
        ...
        currElectricMeter,
        electricRate,
        otherCharges: studentAdj.adjustment + (otherCharges / roomOccupants.length),
        notes: studentAdj.note ? `${studentAdj.note}. ${roomBillNotes}` : roomBillNotes
      };
    ...
    window.dispatchEvent(new CustomEvent('cpatms_db_update', { detail: { key: 'cpatms_bills' } }));
  ...
  const handleApproveSlip = (paymentId: string) => {
    ...
    window.dispatchEvent(new CustomEvent('cpatms_db_update', { detail: { key: 'cpatms_payments' } }));
  ...
  const [newDormGender, setNewDormGender] = useState<'Male' | 'Female' | 'Mixed'>('Mixed');
  ...
  const [editingCapacityValue, setEditingCapacityValue] = useState<number | null>(null);
  ...
  const [newVanStatus, setNewVanStatus] = useState<'active' | 'maintenance' | 'inactive'>('active');
  ...
  const [newTripStatus, setNewTripStatus] = useState<'scheduled' | 'in-progress' | 'completed' | 'cancelled'>('scheduled');

  const [activeTripId, setActiveTripId] = useState<string | null>(null);
  const [assigningSeatNumber, setAssigningSeatNumber] = useState<number | null>(null);
  ...
  const handleDeleteDormitory = (dormId: string) => {
    ...
    const confirmDelete = window.confirm('Are you sure you want to delete this dormitory? All rooms inside will be orphaned or must be deleted.');
    ...
    if (hasOccupancy) {
      triggerToast('Cannot delete dormitory: Some rooms contain currently assigned students.', 'error');
      return;
    }
  ...
  const handleCreateRoom = (e: React.FormEvent) => {
    ...
    if (!newRoomNumber || !newRoomDormId) {
      triggerToast('Room Number and Dormitory selection are required.', 'error');
      return;
    }
    const capVal = Number(newRoomCapacity);
    if (capVal <= 0 || capVal > 8) {
      triggerToast('Room capacity cannot exceed the maximum safety limit of 8 bed spaces.', 'error');
      return;
    }

    const roomsList = mockDB.getRooms();
    const isDuplicate = roomsList.some(r => r.buildingId === newRoomDormId && r.roomNumber === newRoomNumber);
    if (isDuplicate) {
    ...
    triggerToast(`Room ${newRoomNumber} created successfully with capacity of ${capVal}!`);
  ...
  const handleDeleteRoom = (roomId: string) => {
    ...
    if (targetRoom.occupiedCount > 0) {
      triggerToast('Cannot delete room: Room is currently occupied. Please unassign students first.', 'error');
      return;
    }

    const confirmDelete = window.confirm(`Are you sure you want to delete Room ${targetRoom.roomNumber}?`);
    if (!confirmDelete) return;
    ...
    const buildingsList = mockDB.getBuildings();
    const bIndex = buildingsList.findIndex(b => b.id === targetRoom.buildingId);
    if (bIndex !== -1) {
      buildingsList[bIndex] = {
        ...buildingsList[bIndex],
        totalRooms: Math.max(0, (buildingsList[bIndex].totalRooms || 1) - 1),
        totalBeds: Math.max(0, (buildingsList[bIndex].totalBeds || targetRoom.capacity) - targetRoom.capacity)
      };
      mockDB.saveBuildings(buildingsList);
    }
  ...
  const handleSetCapacity = (roomId: string, newCap: number) => {
    ...
    if (newCap < currentRoom.occupiedCount) {
      triggerToast(`Cannot decrease capacity below current occupancy of ${currentRoom.occupiedCount} students.`, 'error');
      return;
    }
    ...
    const buildingsList = mockDB.getBuildings();
    const bIndex = buildingsList.findIndex(b => b.id === currentRoom.buildingId);
    if (bIndex !== -1) {
      buildingsList[bIndex] = {
        ...buildingsList[bIndex],
    ...
    triggerToast(`Room ${currentRoom.roomNumber} capacity updated to ${newCap}!`);
  ...
  const handleAssignStudent = () => {
    ...
    const roomsList = mockDB.getRooms();
    const rIndex = roomsList.findIndex(r => r.id === selectedRoomForAssign.id);
    if (rIndex === -1) return;
  ...
  const handleMoveStudent = () => {
    ...
    const roomsList = mockDB.getRooms();
    const oldRoomIndex = roomsList.findIndex(r => r.id === studentToMove.roomId);
    ...
    triggerToast(`Successfully moved ${student.studentName} to Room ${newRoom.roomNumber}!`);
  ...
  const handleRemoveStudentFromRoom = (studentId: string, roomId: string) => {
    ...
    const assignmentList = mockDB.getRoomAssignments();
    assignmentList.forEach(a => {
      if (a.studentId === studentId && a.roomId === roomId && a.status === 'active') {
        a.status = 'completed';
      }
    });
    mockDB.saveRoomAssignments(assignmentList);
  ...
  const handleCreateTrip = (e: React.FormEvent) => {
    e.preventDefault();
    if (!newTripRoute || !newTripPickup || !newTripDeparture || !newTripVehicleId) {
      triggerToast('Route, Pickup Point, Departure Time, and Van selection are required.', 'error');
      return;
    }
  ...
  const handleAssignStudentToSeat = (e?: React.FormEvent) => {
    ...
    const bookingsForTrip = dbTransportAssignments.filter(a => a.scheduleId === activeTripId && a.status === 'active');
    
    const studentAlreadyBooked = bookingsForTrip.find(b => b.studentId === selectedStudentForSeat);
    const existingOccupantOnSeat = bookingsForTrip.find(b => b.seatNumber === assigningSeatNumber);

    if (existingOccupantOnSeat && existingOccupantOnSeat.studentId !== selectedStudentForSeat) {
    ...
    if (!studentAlreadyBooked && bookingsForTrip.length >= vehicle.capacity) {
      triggerToast(`Cannot assign student. Van capacity (${vehicle.capacity}) exceeded.`, 'error');
      return;
    }

    let currentBookings = mockDB.getTransportAssignments();

    currentBookings = currentBookings.filter(b => !(b.studentId === selectedStudentForSeat && b.scheduleId === activeTripId));
    ...
    triggerToast(`Assigned ${studentObj?.studentName || 'Student'} to Seat #${assigningSeatNumber}!`);
  ...
  return (
    <div className="space-y-6">
        ...
        <div className="space-y-6">
          ...
          <div className="rounded-2xl bg-white p-6 shadow-xs border border-slate-100 dark:bg-zinc-900 dark:border-zinc-800">
            ...
            <p className="mt-1 text-sm text-zinc-500 dark:text-zinc-400">
              Srisavarindhira Thai Red Cross Institute of Nursing Clinical Operations Hub.
            </p>
          </div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
            <div className="rounded-2xl bg-white border border-slate-100 p-5 dark:bg-zinc-900 dark:border-zinc-800">
              <span className="text-xs font-semibold text-zinc-400 uppercase tracking-widest">Active Placements</span>
              <div className="mt-2 flex items-baseline justify-between">
                <span className="text-3xl font-black text-zinc-900 dark:text-white">35</span>
                <span className="text-xs font-bold text-emerald-600 bg-emerald-50 dark:bg-emerald-950/30 px-2 py-0.5 rounded-md">+4 new</span>
              </div>
            </div>
            <div className="rounded-2xl bg-white border border-slate-100 p-5 dark:bg-zinc-900 dark:border-zinc-800">
              <span className="text-xs font-semibold text-zinc-400 uppercase tracking-widest">Total Host Sites</span>
              <div className="mt-2 flex items-baseline justify-between">
                <span className="text-3xl font-black text-zinc-900 dark:text-white">{sites.length}</span>
                <span className="text-xs font-bold text-zinc-500">Chonburi Hub</span>
              </div>
            </div>
            <div className="rounded-2xl bg-white border border-slate-100 p-5 dark:bg-zinc-900 dark:border-zinc-800">
              <span className="text-xs font-semibold text-zinc-400 uppercase tracking-widest">Dorm Beds Occupancy</span>
              <div className="mt-2 flex items-baseline justify-between">
                <span className="text-3xl font-black text-zinc-900 dark:text-white">
                  {(() => {
                    const totalOccupied = dbRooms.reduce((acc, curr) => acc + (curr.occupiedCount || 0), 0);
                    const totalCapacity = dbRooms.reduce((acc, curr) => acc + (curr.capacity || 0), 0);
            ...
            <div className="rounded-2xl bg-white border border-slate-100 p-5 dark:bg-zinc-900 dark:border-zinc-800">
              <span className="text-xs font-semibold text-zinc-400 uppercase tracking-widest">Pending Bills slips</span>
              <div className="mt-2 flex items-baseline justify-between">
                <span className="text-3xl font-black text-zinc-900 dark:text-white">
                  {bills.filter(b => b.status === 'Pending').length}
                </span>
                <span className="text-xs font-bold text-amber-600 bg-amber-50 dark:bg-amber-950/20 px-2 py-0.5 rounded-md">Action Required</span>
              </div>
            </div>
          </div>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
            <div className="lg:col-span-2 space-y-6">
              ...
              <div className="rounded-2xl bg-white border border-slate-100 p-5 dark:bg-zinc-900 dark:border-zinc-800">
                <div className="flex items-center justify-between mb-4">
                  <h3 className="text-sm font-black text-zinc-800 dark:text-white uppercase tracking-wider">Operational Training Groups</h3>
                  <Users className="h-4 w-4 text-zinc-400" />
                </div>
                <div className="space-y-3">
                  {groups.map(g => {
                    const hospitalObj = mockDB.getHospitals().find(h => h.id === g.hospital || h.hospitalNameEN === g.hospital || h.hospitalNameTH === g.hospital);
                    const hospitalName = hospitalObj ? hospitalObj.hospitalNameTH : (g.hospital || 'General');
                    const studentCount = g.studentIds ? g.studentIds.length : 0;
                    return (
                      <div key={g.id} className="flex items-center justify-between p-3.5 rounded-xl bg-slate-50/50 border border-slate-100/50 dark:bg-zinc-900/40 dark:border-zinc-800">
                        <div>
                          <h4 className="text-xs font-black text-zinc-800 dark:text-zinc-200">{g.name}</h4>
                          <p className="text-[10px] text-zinc-400 font-bold mt-0.5">{hospitalName} • {g.startDate} to {g.endDate}</p>
                        </div>
              ...
              <div className="rounded-2xl bg-white border border-slate-100 p-5 dark:bg-zinc-900 dark:border-zinc-800">
                <div className="flex items-center justify-between mb-4">
                  <h3 className="text-sm font-black text-zinc-800 dark:text-white uppercase tracking-wider">Latest Bulletin Posts</h3>
                  <Megaphone className="h-4 w-4 text-zinc-400" />
                </div>
                <div className="space-y-4">
                  {announcements.slice(0, 2).map(a => (
                    <div key={a.id} className="border-l-4 border-red-600 pl-4 py-1">
                      <div className="flex items-center gap-2">
                        <span className={`text-[9px] font-black uppercase px-2 py-0.5 rounded-md ${a.priority === 'Urgent' ? 'bg-red-100 text-red-700' : 'bg-slate-100 text-slate-700 dark:bg-zinc-800 dark:text-zinc-300'}`}>
                          {a.priority}
                        </span>
                        <span className="text-[10px] text-zinc-400 font-bold">{a.date}</span>
                      </div>
                      <h4 className="text-xs font-black text-zinc-800 dark:text-zinc-200 mt-1">{a.title}</h4>
                      <p className="text-[11px] text-zinc-500 dark:text-zinc-400 mt-1 line-clamp-2">{a.content}</p>
                    </div>
                  ))}
                </div>
              </div>
            ...
            <div className="space-y-6">
              <div className="rounded-2xl bg-white border border-slate-100 p-5 dark:bg-zinc-900 dark:border-zinc-800">
                <h3 className="text-xs font-black text-zinc-800 dark:text-white uppercase tracking-wider mb-4">Quick Task Portal</h3>
                <div className="space-y-2">
                  <div className="p-3 bg-red-50/40 rounded-xl border border-red-100/50 text-[11px] font-bold text-red-600 leading-normal dark:bg-red-950/10 dark:border-red-900/20">
                    One-Click allocation of dorms, buses, and evaluation checklists is operational. Use dedicated tabs below to manage.
                  </div>
                  <div className="border border-slate-50 dark:border-zinc-800 p-3 rounded-xl flex items-center justify-between bg-white dark:bg-zinc-950">
                    <span className="text-xs font-bold text-zinc-600 dark:text-zinc-400">Current Term</span>
                    <span className="text-xs font-black text-zinc-900 dark:text-white">{systemSemester}</span>
                  </div>
                </div>
              </div>
            </div>
          ...
          <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 bg-white border border-slate-100 p-6 rounded-2xl dark:bg-zinc-900 dark:border-zinc-800">
            <div>
              ...
              <p className="text-xs text-zinc-500 dark:text-zinc-400 mt-1">
                Administer student deployments, assign teachers, verify hospital placements, and provision dormitories & transit.
              </p>
            </div>
            {!isGroupFormOpen && (
              <button
                onClick={() => { resetGroupForm(); setIsGroupFormOpen(true); }}
            ...
            <div className="rounded-2xl bg-white border border-red-100 dark:border-zinc-800 p-6 dark:bg-zinc-900 shadow-md transition-all duration-300">
              <div className="flex items-center justify-between border-b border-slate-100 dark:border-zinc-800 pb-4 mb-6">
                <h4 className="text-sm font-black text-zinc-900 dark:text-white uppercase tracking-wider flex items-center gap-2">
                  ...
                  {editingGroupId ? 'Edit Training Group Parameters' : 'Provision New Training Group'}
                </h4>
                <button
                  onClick={() => { resetGroupForm(); setIsGroupFormOpen(false); }}
              ...
              <form onSubmit={handleSaveGroup} className="space-y-6">
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                  {/* Group Name */}
                  <div>
                    <label className="block text-[10px] font-black text-zinc-400 uppercase tracking-wider mb-1.5">Group Title / Rotation Name *</label>
                    <input
                      type="text"
                  ...
                  {/* Hospital */}
                  <div>
                    <label className="block text-[10px] font-black text-zinc-400 uppercase tracking-wider mb-1.5">Hospital Clinical Placement *</label>
                    <select
                      value={tgHospital}
                  ...
                  {/* Teacher */}
                  <div>
                    <label className="block text-[10px] font-black text-zinc-400 uppercase tracking-wider mb-1.5">Supervising Instructor *</label>
                    <select
                      value={tgTeacherId}
                  ...
                  {/* Dates */}
                  <div>
                    <label className="block text-[10px] font-black text-zinc-400 uppercase tracking-wider mb-1.5">Deployment Start Date *</label>
                  ...
                  <div>
                    <label className="block text-[10px] font-black text-zinc-400 uppercase tracking-wider mb-1.5">Deployment End Date *</label>
                    <div className="relative">
                      <input
                        type="date"
                        value={tgEndDate}
                  ...
                  {/* Transportation */}
                  <div>
                    <label className="block text-[10px] font-black text-zinc-400 uppercase tracking-wider mb-1.5">Transit Schedule & Vehicle Allocation</label>
                    <select
                      value={tgTransportationId}
                      onChange={(e) => setTgTransportationId(e.target.value)}
                      className="w-full rounded-xl border border-slate-100 bg-white px-3 py-2 text-xs outline-hidden focus:ring-1 focus:ring-red-600 dark:border-zinc-800 dark:bg-zinc-900 text-zinc-800 dark:text-zinc-100"
                    >
                      <option value="">-- No Transportation (Self Travel) --</option>
                      {mockDB.getTransportSchedules().map(ts => {
                        const vehicle = mockDB.getVehicles().find(v => v.id === ts.vehicleId);
                        const driver = mockDB.getDrivers().find(d => d.id === ts.driverId);
                        return (
                          <option key={ts.id} value={ts.id}>
                            {ts.route} ({ts.departureTime}) - {vehicle?.model || 'Shuttle'} • {driver?.name || 'Driver'}
                          </option>
                        );
                      })}
                    </select>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6 pt-2">
                  ...
                  <div className="border border-slate-100 dark:border-zinc-800 p-4 rounded-2xl bg-slate-50/30 dark:bg-zinc-950/20">
                    <label className="block text-[10px] font-black text-zinc-500 uppercase tracking-wider mb-2">Dormitory Room Allocation (Multiple Select)</label>
                    <div className="max-h-48 overflow-y-auto space-y-2 pr-1 scrollbar-thin">
                      {mockDB.getRooms().map(room => {
                        const bldg = mockDB.getBuildings().find(b => b.id === room.buildingId);
                        const isSelected = tgDormitoryIds.includes(room.id);
                        return (
                          <label key={room.id} className="flex items-center justify-between p-2 rounded-xl border border-slate-100 dark:border-zinc-800 bg-white dark:bg-zinc-900 hover:bg-slate-50 dark:hover:bg-zinc-800/50 cursor-pointer text-xs">
                            <div className="flex items-center gap-2">
                              <input
                                type="checkbox"
                                checked={isSelected}
                                onChange={() => {
                                  if (isSelected) {
                                    setTgDormitoryIds(tgDormitoryIds.filter(id => id !== room.id));
                                  } else {
                                    setTgDormitoryIds([...tgDormitoryIds, room.id]);
                                  }
                                }}
                            ...
                            <div className="text-[10px] text-zinc-400 font-bold">
                              {bldg?.buildingName || 'Main Campus Dorm'} • Capacity: {room.capacity}
                            </div>
                          </label>
                        );
                      })}
                  ...
                  <div className="border border-slate-100 dark:border-zinc-800 p-4 rounded-2xl bg-slate-50/30 dark:bg-zinc-950/20">
                    <label className="block text-[10px] font-black text-zinc-500 uppercase tracking-wider mb-2">Nursing Students Enrollment (Multiple Select)</label>
                    <div className="max-h-48 overflow-y-auto space-y-2 pr-1 scrollbar-thin">
                      {mockDB.getStudents().map(stud => {
                        const isSelected = tgStudentIds.includes(stud.id);
                        return (
                          <label key={stud.id} className="flex items-center justify-between p-2 rounded-xl border border-slate-100 dark:border-zinc-800 bg-white dark:bg-zinc-900 hover:bg-slate-50 dark:hover:bg-zinc-800/50 cursor-pointer text-xs">
                            <div className="flex items-center gap-2">
                              <input
                                type="checkbox"
                                checked={isSelected}
                                onChange={() => {
                                  if (isSelected) {
                                    setTgStudentIds(tgStudentIds.filter(id => id !== stud.id));
                                  } else {
          ...
          <div className="rounded-2xl bg-white border border-slate-100 p-6 dark:bg-zinc-900 dark:border-zinc-800">
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-6">
              ...
              <div className="flex flex-col sm:flex-row gap-3">
                ...
                >
                  ...
                  {mockDB.getHospitals().map(h => (
                    <option key={h.id} value={h.id}>{h.shortName || h.hospitalNameEN}</option>
                  ))}
                </select>
              </div>
            </div>
            ...
            {groups.length === 0 ? (
              <div className="flex flex-col items-center justify-center py-12 border-2 border-dashed border-slate-100 dark:border-zinc-800 rounded-2xl">
                ...
                <p className="text-xs font-semibold text-zinc-400 uppercase">No cohorts registered</p>
                <p className="text-[11px] text-zinc-400 mt-0.5">Click 'Create Training Group' to start deploying nursing students.</p>
              </div>
            ) : (
              <div className="grid grid-cols-1 gap-6">
                {groups
                  .filter(g => {
                    const hospitalObj = mockDB.getHospitals().find(h => h.id === g.hospital || h.hospitalNameEN === g.hospital || h.hospitalNameTH === g.hospital);
                    const hospitalName = hospitalObj ? `${hospitalObj.hospitalNameTH} ${hospitalObj.hospitalNameEN}` : g.hospital;
                    const matchesSearch = g.name.toLowerCase().includes(tgSearch.toLowerCase()) || hospitalName.toLowerCase().includes(tgSearch.toLowerCase());
                    const matchesFilter = tgFilterHospital ? (g.hospital === tgFilterHospital) : true;
                    return matchesSearch && matchesFilter;
                  })
                  .map(g => {
                    const teacher = mockDB.getTeachers().find(t => t.id === g.teacherId);
                    const hospital = mockDB.getHospitals().find(h => h.id === g.hospital);
                    const transit = mockDB.getTransportSchedules().find(ts => ts.id === g.transportationId);
                    const studentsCount = g.studentIds ? g.studentIds.length : 0;
                    
                    return (
                      <div key={g.id} className="relative group overflow-hidden border border-slate-100 dark:border-zinc-800 rounded-2xl p-5 bg-slate-50/20 dark:bg-zinc-950/15 hover:border-red-100 dark:hover:border-red-900/40 transition duration-300">
                        {/* Upper row */}
                        <div className="flex flex-col md:flex-row md:items-start justify-between gap-4">
                          <div>
                            ...
                            <h4 className="text-sm font-black text-zinc-800 dark:text-zinc-200 mt-2">{g.name}</h4>
                            <p className="text-xs text-zinc-500 dark:text-zinc-400 mt-1 font-semibold flex items-center gap-1">
                              <MapPin className="h-3.5 w-3.5 text-red-500" />
                              {hospital?.hospitalNameTH || g.hospital} • {hospital?.province || 'Hospital Placement'}
                            </p>
                          </div>
                        ...
                        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 mt-5 pt-4 border-t border-slate-100 dark:border-zinc-800/50">
                          <div>
                            <span className="block text-[9px] font-black text-zinc-400 uppercase tracking-widest">Supervisor</span>
                            <span className="text-xs font-semibold text-zinc-700 dark:text-zinc-300 mt-0.5 block">
                              {teacher?.name || 'Unassigned Instructor'}
                            </span>
                            <span className="text-[10px] text-zinc-400 block mt-0.5">{teacher?.department || 'Faculty of Nursing'}</span>
                          </div>

                          <div>
                            <span className="block text-[9px] font-black text-zinc-400 uppercase tracking-widest">Term Period</span>
                            ...
                            <span className="text-[10px] text-zinc-400 block mt-0.5">Clinical practice cycle</span>
                          </div>

                          <div>
                            <span className="block text-[9px] font-black text-zinc-400 uppercase tracking-widest">Lodging / Dormitories</span>
                            <span className="text-xs font-semibold text-zinc-700 dark:text-zinc-300 mt-0.5 block flex items-center gap-1">
                              <Bed className="h-3.5 w-3.5 text-zinc-400" />
                              {g.dormitoryIds && g.dormitoryIds.length > 0 ? (
                                <span>{g.dormitoryIds.length} Rooms Assigned</span>
                              ) : (
                                <span className="text-zinc-400 italic">No rooms provisioned</span>
                              )}
                            </span>
                            {g.dormitoryIds && g.dormitoryIds.length > 0 && (
                              <span className="text-[10px] text-zinc-400 block mt-0.5">
                                Rooms: {g.dormitoryIds.map(id => {
                          ...
                          <div>
                            <span className="block text-[9px] font-black text-zinc-400 uppercase tracking-widest">Transportation</span>
                            <span className="text-xs font-semibold text-zinc-700 dark:text-zinc-300 mt-0.5 block flex items-center gap-1">
                              <Bus className="h-3.5 w-3.5 text-zinc-400" />
                              {transit ? transit.route : <span className="text-zinc-400 italic">Self-commute</span>}
                            </span>
                            {transit && (
                              <span className="text-[10px] text-zinc-400 block mt-0.5">Shuttle departs at {transit.departureTime}</span>
                            )}
                          </div>
                        </div>
                        ...
                        <div className="mt-5 pt-4 border-t border-slate-100 dark:border-zinc-800/50 flex flex-wrap items-center gap-2">
                          <span className="text-[9px] font-black text-zinc-400 uppercase tracking-widest mr-1">Students ({studentsCount}):</span>
                          {g.studentIds && g.studentIds.length > 0 ? (
                            g.studentIds.map(id => {
                              const student = mockDB.getStudents().find(s => s.id === id);
                              return (
                                <span key={id} className="inline-flex items-center rounded-lg bg-slate-100 dark:bg-zinc-800 px-2 py-1 text-xs font-bold text-zinc-600 dark:text-zinc-300 border border-slate-200/40 dark:border-zinc-700/50">
                                  {student?.studentName || id}
                                </span>
                              );
                            })
                          ) : (
                            <span className="text-xs text-zinc-400 italic">No students allocated yet.</span>
                          )}
                        </div>
                      </div>
                    );
      ...
      {activeTab === 'sites' && (
        <div className="space-y-6">
          <div className="rounded-2xl bg-white border border-slate-100 p-6 dark:bg-zinc-900 dark:border-zinc-800">
            <h3 className="text-lg font-black text-zinc-900 dark:text-white mb-2">Hospital Partners & Wards</h3>
            <p className="text-xs text-zinc-500 dark:text-zinc-400 mb-6">Manage approved healthcare clinical centers and student capacities.</p>

            {/* Add Site Form */}
            <form onSubmit={handleAddSite} className="grid grid-cols-1 md:grid-cols-4 gap-4 mb-6 p-4 bg-slate-50/50 dark:bg-zinc-950/30 rounded-2xl border border-slate-100 dark:border-zinc-800">
              <div className="md:col-span-2">
                <label className="block text-[10px] font-black text-zinc-400 uppercase mb-1">Hospital / Clinic Name</label>
                <input 
                  type="text" 
              ...
              <div>
                <label className="block text-[10px] font-black text-zinc-400 uppercase mb-1">Location Province</label>
                <input 
                  type="text" 
              ...
              <div className="flex items-end">
                <button type="submit" className="w-full flex items-center justify-center gap-1.5 rounded-xl bg-red-600 text-xs font-black text-white py-2.5 shadow-sm hover:bg-red-500 transition cursor-pointer">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
              {sites.map(s => (
                <div key={s.id} className="rounded-xl border border-slate-100 p-4 dark:border-zinc-800 bg-slate-50/30 dark:bg-zinc-900/10 flex flex-col justify-between">
                  <div>
                    <div className="flex items-center justify-between">
                      <span className="text-[10px] font-black uppercase text-red-600">{s.location}</span>
                      <span className={`text-[10px] px-2 py-0.5 rounded-md font-bold ${s.status === 'Active' ? 'bg-emerald-50 text-emerald-700 dark:bg-emerald-950/20' : 'bg-amber-50 text-amber-700 dark:bg-amber-950/20'}`}>
                        {s.status}
                      </span>
                    </div>
                    <h4 className="text-sm font-black text-zinc-900 dark:text-white mt-2">{s.name}</h4>
                  </div>
                  <div className="mt-4 pt-3 border-t border-slate-100/50 dark:border-zinc-800 flex items-center justify-between text-xs font-bold">
                    ...
                    <span className="text-zinc-900 dark:text-zinc-200">{s.occupied} / {s.capacity} Students</span>
                  </div>
                </div>
              ))}
            </div>
      ...
      {/* ==================== 4. DORMITORIES TAB ==================== */}
      {activeTab === 'dorms' && (() => {
        const dormitories = dbBuildings.filter(b => b.buildingType === 'Dormitory');
        ...
        const filteredRooms = dbRooms.filter(room => {
          ...
          const belongsToDorm = dormitories.some(d => d.id === room.buildingId);
          if (!belongsToDorm) return false;

          // Dorm filter
          if (selectedDormId !== 'all' && room.buildingId !== selectedDormId) return false;

          // Gender filter
          if (selectedRoomGender !== 'all' && room.gender !== selectedRoomGender) return false;
        ...
        const totalCapacity = dbRooms.filter(r => dormitories.some(d => d.id === r.buildingId))
          .reduce((acc, curr) => acc + (curr.capacity || 0), 0);
        const totalOccupied = dbRooms.filter(r => dormitories.some(d => d.id === r.buildingId))
          .reduce((acc, curr) => acc + (curr.occupiedCount || 0), 0);
        const overallOccupancyPercent = totalCapacity > 0 ? Math.round((totalOccupied / totalCapacity) * 100) : 0;

        // Resolve unassigned students
        const unassignedStudents = dbStudents.filter(s => !s.roomId && s.status === 'active');
          ...
          <div className="space-y-6">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
              <div className="rounded-2xl bg-white border border-slate-100 p-5 dark:bg-zinc-900 dark:border-zinc-800 shadow-sm">
                <span className="text-[10px] font-black text-zinc-400 uppercase tracking-wider block">Total Dormitories</span>
                <div className="mt-2 flex items-baseline justify-between">
                  <span className="text-2xl font-black text-zinc-900 dark:text-white">{totalDormitoriesCount}</span>
                  <span className="text-[10px] font-semibold text-zinc-500">Buildings Configured</span>
                </div>
              </div>

              <div className="rounded-2xl bg-white border border-slate-100 p-5 dark:bg-zinc-900 dark:border-zinc-800 shadow-sm">
                <span className="text-[10px] font-black text-zinc-400 uppercase tracking-wider block">Total Rooms</span>
                <div className="mt-2 flex items-baseline justify-between">
                  <span className="text-2xl font-black text-zinc-900 dark:text-white">
                    {dbRooms.filter(r => dormitories.some(d => d.id === r.buildingId)).length}
                  </span>
                  <span className="text-[10px] font-semibold text-emerald-600">Active Residential Units</span>
                </div>
              </div>

              <div className="rounded-2xl bg-white border border-slate-100 p-5 dark:bg-zinc-900 dark:border-zinc-800 shadow-sm">
                <span className="text-[10px] font-black text-zinc-400 uppercase tracking-wider block">Bed Capacity</span>
                <div className="mt-2 flex items-baseline justify-between">
                  <span className="text-2xl font-black text-zinc-900 dark:text-white">
                    {totalOccupied} / {totalCapacity}
                  </span>
                  <span className="text-[10px] font-bold text-zinc-500">Beds Assigned</span>
                </div>
              </div>
              ...
              <div className="rounded-2xl bg-white border border-slate-100 p-5 dark:bg-zinc-900 dark:border-zinc-800 shadow-sm flex flex-col justify-between">
                <div>
                  <span className="text-[10px] font-black text-zinc-400 uppercase tracking-wider block">Overall Occupancy Rate</span>
                  <div className="mt-2 flex items-baseline justify-between">
                    <span className="text-2xl font-black text-zinc-900 dark:text-white">{overallOccupancyPercent}%</span>
                    <span className="text-[10px] font-black text-red-600 uppercase tracking-widest">Live stats</span>
                  </div>
                </div>
            ...
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 p-4 rounded-2xl bg-slate-50/50 border border-slate-100 dark:bg-zinc-950/20 dark:border-zinc-800">
              <div className="space-y-1">
                <h3 className="text-sm font-black text-zinc-900 dark:text-white">Residential Configurator</h3>
                <p className="text-[11px] text-zinc-400 font-medium">Create new dorms, add housing units, set bed capacities, and supervise placements.</p>
              </div>
            ...
            {/* 4.3. CREATION FORMS */}
            {showCreateDormForm && (
              <form onSubmit={handleCreateDormitory} className="p-6 rounded-2xl bg-white border border-slate-100 dark:bg-zinc-900 dark:border-zinc-800 space-y-4 shadow-sm animate-fadeIn">
                <div className="flex justify-between items-center border-b border-slate-50 dark:border-zinc-800 pb-3">
                  <h4 className="text-sm font-black text-zinc-900 dark:text-white uppercase tracking-wider flex items-center gap-1.5">
                    <Plus className="h-4 w-4 text-red-600" /> New Dormitory Building
                  </h4>
                  <button type="button" onClick={() => setShowCreateDormForm(false)} className="text-zinc-400 hover:text-zinc-600">
                    <X className="h-4 w-4" />
                  </button>
                </div>
                <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                  <div>
                    <label className="block text-[10px] font-black text-zinc-400 uppercase mb-1">Building Code</label>
                    <input 
                      type="text" 
                  ...
                  <div>
                    <label className="block text-[10px] font-black text-zinc-400 uppercase mb-1">Building Name</label>
                    <input 
                      type="text" 
                  ...
                  <div>
                    <label className="block text-[10px] font-black text-zinc-400 uppercase mb-1">Gender Wing Designation</label>
                    <select 
                      value={newDormGender} 
                      onChange={(e) => setNewDormGender(e.target.value as any)}
                      className="w-full rounded-xl border border-slate-100 bg-white px-3 py-2 text-xs outline-hidden focus:ring-1 focus:ring-red-600 dark:border-zinc-800 dark:bg-zinc-900 dark:text-white"
                    >
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                  <div>
                    <label className="block text-[10px] font-black text-zinc-400 uppercase mb-1">Residential Floors</label>
                    <input 
                      type="number" 
                      min="1" 
                      max="10"
                      value={newDormFloors} 
                      onChange={(e) => setNewDormFloors(Number(e.target.value))}
                      className="w-full rounded-xl border border-slate-100 bg-white px-3 py-2 text-xs outline-hidden focus:ring-1 focus:ring-red-600 dark:border-zinc-800 dark:bg-zinc-900 dark:text-white"
                      required
                    />
                  </div>
                  <div className="md:col-span-2">
                    <label className="block text-[10px] font-black text-zinc-400 uppercase mb-1">Address / Location Description</label>
                    <input 
                      type="text" 
                ...
                <div>
                  <label className="block text-[10px] font-black text-zinc-400 uppercase mb-1">General Notes & Guidelines</label>
                  <textarea 
            ...
            {showCreateRoomForm && (
              <form onSubmit={handleCreateRoom} className="p-6 rounded-2xl bg-white border border-slate-100 dark:bg-zinc-900 dark:border-zinc-800 space-y-4 shadow-sm animate-fadeIn">
                <div className="flex justify-between items-center border-b border-slate-50 dark:border-zinc-800 pb-3">
                  <h4 className="text-sm font-black text-zinc-900 dark:text-white uppercase tracking-wider flex items-center gap-1.5">
                    <Plus className="h-4 w-4 text-red-600" /> New Residential Room Unit
                  </h4>
                  <button type="button" onClick={() => setShowCreateRoomForm(false)} className="text-zinc-400 hover:text-zinc-600">
                    <X className="h-4 w-4" />
                  </button>
                </div>
                <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
                  <div>
                    <label className="block text-[10px] font-black text-zinc-400 uppercase mb-1">Select Dormitory</label>
                    <select 
                      value={newRoomDormId} 
                      onChange={(e) => {
                        setNewRoomDormId(e.target.value);
                        const selectedDorm = dormitories.find(d => d.id === e.target.value);
                        if (selectedDorm) {
                    ...
                    >
                      ...
                      {dormitories.map(d => (
                        <option key={d.id} value={d.id}>{d.buildingName} ({d.buildingCode})</option>
                      ))}
                    </select>
                  </div>
                  <div>
                    <label className="block text-[10px] font-black text-zinc-400 uppercase mb-1">Room Number</label>
                    <input 
                      type="text" 
                  ...
                  <div>
                    <label className="block text-[10px] font-black text-zinc-400 uppercase mb-1">Gender Restriction</label>
                    <select 
                      value={newRoomGender} 
                  ...
                  <div>
                    <label className="block text-[10px] font-black text-zinc-400 uppercase mb-1">Bed Capacity</label>
                    <input 
                      type="number" 
                      min="1" 
                      max="8"
                      ...
                      required
                    />
                    <span className="block text-[9px] text-zinc-400 mt-1 font-semibold">Maximum limit: 8 beds per room</span>
                  </div>
                </div>
            ...
            <div className="rounded-2xl bg-white border border-slate-100 p-5 dark:bg-zinc-900 dark:border-zinc-800 shadow-sm flex flex-col md:flex-row gap-4 items-center justify-between">
              ...
              <div className="flex flex-col sm:flex-row gap-3 w-full md:w-auto">
                <div className="flex items-center gap-1.5 w-full sm:w-auto">
                  <span className="text-[10px] font-black uppercase text-zinc-400 shrink-0">Dormitory:</span>
                  <select 
                ...
                <div className="flex items-center gap-1.5 w-full sm:w-auto">
                  <span className="text-[10px] font-black uppercase text-zinc-400 shrink-0">Gender restriction:</span>
                  <select 
            ...
            <div className="grid grid-cols-1 xl:grid-cols-2 gap-6">
              {filteredRooms.length > 0 ? (
                filteredRooms.map(room => {
                  const dorm = dormitories.find(d => d.id === room.buildingId);
                  const roomStudents = getStudentsInRoom(room.id);
                  const occupancyRate = Math.round((room.occupiedCount / room.capacity) * 100);

                  return (
                    <div key={room.id} className="rounded-2xl border border-slate-100 bg-white p-5 dark:border-zinc-800 dark:bg-zinc-900 shadow-xs flex flex-col justify-between">
                      <div>
                        {/* Card Header */}
                        <div className="flex items-start justify-between border-b border-slate-50 dark:border-zinc-800 pb-3 mb-4">
                          <div>
                            <div className="flex items-center gap-2">
                              <h4 className="text-sm font-black text-zinc-900 dark:text-white">Room {room.roomNumber}</h4>
                            ...
                            <p className="text-[10px] text-zinc-400 font-bold mt-1">{dorm?.buildingName || 'Main Residence Hall'}</p>
                          </div>
                            ...
                            {editingRoomId === room.id ? (
                              <div className="flex items-center gap-1 bg-slate-50 p-1 rounded-lg dark:bg-zinc-950">
                                <input 
                                  ...
                                  min="1" 
                                  max="8"
                                  value={editingCapacityValue !== null ? editingCapacityValue : room.capacity}
                                  onChange={(e) => setEditingCapacityValue(Number(e.target.value))}
                                ...
                                />
                                <button 
                                  onClick={() => handleSetCapacity(room.id, editingCapacityValue !== null ? editingCapacityValue : room.capacity)}
                          ...
                          {Array.from({ length: room.capacity }).map((_, idx) => {
                            ...
                            return (
                              ...
                              >
                                ...
                                <span className="block text-[9px] font-black">Bed {idx + 1}</span>
                                <span className="block text-[8px] font-bold truncate max-w-[80px] mx-auto mt-1 uppercase">
                                  {student ? student.studentName.split(' ')[0] : 'Available'}
                                </span>
                              </div>
                            );
                          })}
                      ...
                      <div className="border-t border-slate-50 dark:border-zinc-800 pt-4 space-y-2">
                        <div className="flex items-center justify-between">
                          <span className="text-[10px] font-black uppercase text-zinc-400 tracking-wider">Assigned Roommates</span>
                        ...
                        <div className="space-y-1.5">
                          {roomStudents.length > 0 ? (
                            roomStudents.map((stud) => (
                              <div key={stud.id} className="flex items-center justify-between text-xs p-2 rounded-lg bg-slate-50/50 dark:bg-zinc-950/40 border border-slate-100/10">
                                <div className="min-w-0">
                                  <span className="font-bold text-zinc-800 dark:text-zinc-200 block truncate">{stud.studentName}</span>
                                  <span className="text-[9px] text-zinc-400 block mt-0.5">ID: {stud.studentNumber} • Sec {stud.section}</span>
                                </div>
                                <div className="flex items-center gap-1.5">
                                  <button 
                                    onClick={() => {
                                  ...
                                  <button 
                                    onClick={() => handleRemoveStudentFromRoom(stud.id, room.id)}
                            ...
                            <p className="text-xs text-zinc-400 italic py-2 text-center">Empty Unit. Bed spaces are completely available.</p>
                          )}
                        </div>
                      </div>
                    </div>
                ...
                <div className="col-span-2 text-center py-16 border-2 border-dashed border-slate-100 dark:border-zinc-800 rounded-2xl bg-white dark:bg-zinc-900">
                  ...
                  <p className="text-xs text-zinc-600 dark:text-zinc-400 font-bold">No lodging rooms match your filters.</p>
                  <p className="text-[10px] text-zinc-400 mt-1 max-w-sm mx-auto">Try resetting the room filters or search query, or configure a new residential unit.</p>
                </div>
              )}
            </div>
                ...
                <div className="bg-white dark:bg-zinc-900 rounded-2xl border border-slate-100 dark:border-zinc-800 w-full max-w-md p-6 space-y-5 shadow-lg">
                  <div className="flex justify-between items-center border-b border-slate-50 dark:border-zinc-800 pb-3">
                    <h4 className="text-sm font-black text-zinc-900 dark:text-white uppercase tracking-wider">
                      Assign Student to Room {selectedRoomForAssign.roomNumber}
                    </h4>
                    <button 
                      type="button" 
                      onClick={() => {
                  ...
                  <div className="space-y-4">
                    <div>
                      <label className="block text-[10px] font-black text-zinc-400 uppercase mb-1">Search Unassigned Students</label>
                      <div className="relative">
                        <Search className="absolute left-3 top-2.5 h-3.5 w-3.5 text-zinc-400" />
                        <input 
                          type="text" 
                    ...
                    <div className="max-h-56 overflow-y-auto space-y-2 border border-slate-50 dark:border-zinc-800 p-2 rounded-xl bg-slate-50/30 dark:bg-zinc-950/20">
                      {unassignedStudents
                        .filter(s => {
                          ...
                          return s.studentName.toLowerCase().includes(studentSearchQuery.toLowerCase()) || 
                                 s.studentNumber.toLowerCase().includes(studentSearchQuery.toLowerCase());
                        })
                        .map(s => (
                          <button
                            key={s.id}
                          ...
                          >
                            <div>
                              <span className="block font-bold text-xs">{s.studentName}</span>
                              <span className="block text-[10px] text-zinc-400 mt-0.5">ID: {s.studentNumber} • Sec {s.section}</span>
                            </div>
                            <span className="text-[10px] font-black uppercase text-zinc-400">{s.academicYear}</span>
                          </button>
                        ))}
                      ...
                      {unassignedStudents.filter(s => {
                        ...
                        <p className="text-xs text-zinc-400 text-center py-6 italic">No unassigned active students found.</p>
                      )}
                    </div>
                  </div>
                ...
                <div className="bg-white dark:bg-zinc-900 rounded-2xl border border-slate-100 dark:border-zinc-800 w-full max-w-md p-6 space-y-5 shadow-lg">
                  ...
                  <div className="space-y-4">
                    <div className="p-3.5 bg-slate-50/50 dark:bg-zinc-950/20 border border-slate-100 dark:border-zinc-800 rounded-xl space-y-2">
                      <span className="block text-[10px] font-black text-zinc-400 uppercase tracking-widest">Current Placement</span>
                      <p className="text-xs font-bold text-zinc-800 dark:text-zinc-200">
                        Room {moveFromRoom.roomNumber} • {dormitories.find(d => d.id === moveFromRoom.buildingId)?.buildingName}
                      </p>
                    </div>

                    <div>
                      <label className="block text-[10px] font-black text-zinc-400 uppercase mb-1.5">Select Destination Room</label>
                      <select 
                        value={moveToRoomId} 
                        ...
                        required
                      >
                        <option value="">-- Choose Available Destination Room --</option>
                        {dbRooms
                          .filter(r => {
                            // Room belongs to a dormitory
                            const belongs = dormitories.some(d => d.id === r.buildingId);
                            // Avoid current room
                            const isDifferent = r.id !== moveFromRoom.id;
                            // Has space available
                            const hasSpace = (r.occupiedCount || 0) < r.capacity;
                            return belongs && isDifferent && hasSpace;
                          })
                          .map(r => {
                            const bd = dormitories.find(d => d.id === r.buildingId);
                            return (
                              <option key={r.id} value={r.id}>
                                Room {r.roomNumber} ({bd?.buildingCode}) - {r.occupiedCount}/{r.capacity} beds ({r.gender})
                              </option>
                            );
                          })}
                      </select>
        ...
        <div className="space-y-6">
          ...
          <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 bg-white border border-slate-100 p-6 rounded-2xl dark:bg-zinc-900 dark:border-zinc-800">
            <div>
              <h3 className="text-xl font-black text-zinc-900 dark:text-white flex items-center gap-2">
                <Bus className="h-5 w-5 text-red-600" /> Transportation Management
              </h3>
              <p className="text-xs text-zinc-500 dark:text-zinc-400 mt-1">
                Configure transit fleets, plan commute schedules, assign student seats, and monitor occupancy limit validation.
              </p>
            </div>
          ...
          <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
            <div className="bg-white border border-slate-100 p-4 rounded-xl dark:bg-zinc-900 dark:border-zinc-800">
              <span className="text-[10px] font-black uppercase text-zinc-400 tracking-wider">Total Fleet Size</span>
              <p className="text-2xl font-black text-zinc-900 dark:text-white mt-1">{dbVehicles.length} Vans</p>
            </div>
            <div className="bg-white border border-slate-100 p-4 rounded-xl dark:bg-zinc-900 dark:border-zinc-800">
              <span className="text-[10px] font-black uppercase text-zinc-400 tracking-wider">Active Routes</span>
              <p className="text-2xl font-black text-zinc-900 dark:text-white mt-1">
                {dbTransportSchedules.filter(t => t.status !== 'cancelled').length} Trips
              </p>
            </div>
            <div className="bg-white border border-slate-100 p-4 rounded-xl dark:bg-zinc-900 dark:border-zinc-800">
              <span className="text-[10px] font-black uppercase text-zinc-400 tracking-wider">Booked Passengers</span>
              <p className="text-2xl font-black text-zinc-900 dark:text-white mt-1">
                {dbTransportAssignments.length} Students
              </p>
            </div>
            <div className="bg-white border border-slate-100 p-4 rounded-xl dark:bg-zinc-900 dark:border-zinc-800">
              <span className="text-[10px] font-black uppercase text-zinc-400 tracking-wider">Average Occupancy</span>
              <p className="text-2xl font-black text-zinc-900 dark:text-white mt-1">
                {(() => {
                  const totalCapacity = dbVehicles.reduce((acc, v) => acc + (v.capacity || 0), 0);
          ...
          {/* CREATE VAN FORM */}
          {showCreateVanForm && (
            <form onSubmit={handleCreateVan} className="bg-slate-50 border border-slate-100 p-6 rounded-2xl space-y-4 dark:bg-zinc-900 dark:border-zinc-850">
              <div className="flex items-center justify-between border-b border-slate-100 pb-3 dark:border-zinc-800">
                ...
                <button type="button" onClick={() => setShowCreateVanForm(false)} className="text-zinc-400 hover:text-zinc-600 dark:hover:text-zinc-300">
                  <X className="h-4 w-4" />
                </button>
              </div>
              <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
          ...
          {/* CREATE TRIP FORM */}
          {showCreateTripForm && (
            <form onSubmit={handleCreateTrip} className="bg-slate-50 border border-slate-100 p-6 rounded-2xl space-y-4 dark:bg-zinc-900 dark:border-zinc-850">
              <div className="flex items-center justify-between border-b border-slate-100 pb-3 dark:border-zinc-800">
                <h4 className="text-xs font-black uppercase text-zinc-800 dark:text-zinc-250 tracking-wider">
                  Schedule New Route Trip
                </h4>
                <button type="button" onClick={() => setShowCreateTripForm(false)} className="text-zinc-400 hover:text-zinc-600 dark:hover:text-zinc-300">
                  <X className="h-4 w-4" />
                </button>
              </div>
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                  ...
                  >
                    ...
                    {dbVehicles.map(v => (
                      <option key={v.id} value={v.id} disabled={v.status !== 'active'}>
                        {v.model} ({v.plateNumber}) - Cap {v.capacity} {v.status !== 'active' ? '[Maint]' : ''}
                      </option>
                    ))}
                  </select>
                </div>
            ...
            <div className="lg:col-span-5 space-y-6">
              ...
              <div className="bg-white border border-slate-100 p-5 rounded-2xl dark:bg-zinc-900 dark:border-zinc-800">
                <h4 className="text-sm font-black text-zinc-900 dark:text-white mb-3 flex items-center gap-2">
                  <ShieldCheck className="h-4 w-4 text-emerald-600" /> Fleets (Vans)
                </h4>
                <div className="space-y-3 max-h-[250px] overflow-y-auto pr-1">
                  {dbVehicles.map(v => {
                    const statusColors = v.status === 'active' 
                    ...
                    return (
                      <div key={v.id} className="border border-slate-100 rounded-xl p-3 flex items-center justify-between dark:border-zinc-800/80">
                        <div className="space-y-1">
                          <div className="flex items-center gap-2">
                            <span className="font-bold text-xs text-zinc-900 dark:text-white">{v.model}</span>
                            <span className="text-[10px] font-black bg-slate-100 px-2 py-0.5 rounded text-zinc-600 dark:bg-zinc-800 dark:text-zinc-300">
                              {v.plateNumber}
                            </span>
                          </div>
                          <p className="text-[10px] text-zinc-400 font-bold">Capacity: {v.capacity} Seats</p>
                        </div>
                        <span className={`text-[9px] font-black uppercase px-2 py-0.5 rounded-full ${statusColors}`}>
                          {v.status}
                        </span>
                      </div>
                    );
              ...
              <div className="bg-white border border-slate-100 p-5 rounded-2xl dark:bg-zinc-900 dark:border-zinc-800">
                ...
                <div className="space-y-3">
                  {dbTransportSchedules.map(t => {
                    const bookingsCount = dbTransportAssignments.filter(a => a.scheduleId === t.id && a.status === 'active').length;
                    const van = dbVehicles.find(v => v.id === t.vehicleId);
                    const cap = van?.capacity || 14;
                    const fillPercent = Math.min(100, Math.round((bookingsCount / cap) * 100));
                    ...
                    return (
                      ...
                      >
                        <div className="flex items-start justify-between gap-2">
                          <div>
                            <h5 className="text-xs font-black text-zinc-800 dark:text-white">{t.route}</h5>
                            <p className="text-[10px] text-zinc-400 mt-0.5 font-bold flex items-center gap-1">
                              <MapPin className="h-3 w-3" /> Pickup: {(t as any).pickupLocation || 'Campus Lobby'}
                            </p>
                          </div>
                        ...
                        <div className="mt-3.5 space-y-1.5">
                          <div className="flex justify-between text-[10px] font-black uppercase text-zinc-500">
                            ...
                            <span className={bookingsCount >= cap ? 'text-red-500' : 'text-zinc-600 dark:text-zinc-300'}>
              ...
              {activeTripId ? (() => {
                  ...
                  <div className="bg-white border border-slate-100 p-6 rounded-2xl dark:bg-zinc-900 dark:border-zinc-800 space-y-6">
                    ...
                    <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 border-b border-slate-100 pb-4 dark:border-zinc-800">
                      <div>
                        ...
                        <p className="text-[10px] text-zinc-400 font-bold mt-0.5">
                          Departure: {trip.departureTime} • Pickup: {(trip as any).pickupLocation || 'Campus Hub'}
                        </p>
                      </div>
                      <div className="bg-slate-50 dark:bg-zinc-950 p-2.5 rounded-xl border border-slate-100 dark:border-zinc-850 text-xs font-bold text-zinc-500">
                        <p>Fleet: <span className="text-zinc-800 dark:text-white">{vehicle?.model}</span></p>
                        <p className="mt-0.5">Plate: <span className="text-zinc-800 dark:text-white">{vehicle?.plateNumber}</span></p>
                      </div>
                    </div>
                    ...
                    <div>
                      <div className="flex items-center justify-between mb-4">
                        <span className="text-xs font-black text-zinc-800 dark:text-white flex items-center gap-1.5">
                          <Users className="h-4 w-4 text-zinc-500" /> Interactive Cabin Layout
                        </span>
                        <div className="flex items-center gap-3 text-[10px] font-bold text-zinc-400">
                          <span className="flex items-center gap-1"><span className="h-2 w-2 rounded-full bg-emerald-500 inline-block" /> Free</span>
                          <span className="flex items-center gap-1"><span className="h-2 w-2 rounded-full bg-zinc-800 inline-block" /> Occupied</span>
                        </div>
                      </div>
                      ...
                      <div className="bg-slate-50/50 rounded-2xl p-6 border border-slate-100 dark:bg-zinc-950/40 dark:border-zinc-850 flex flex-col items-center">
                        ...
                        <div className="grid grid-cols-3 gap-3.5 w-full max-w-sm">
                          ...
                          <div className="col-span-1 bg-zinc-100 dark:bg-zinc-900/40 border border-dashed border-slate-300 dark:border-zinc-800 p-2.5 rounded-xl flex flex-col items-center justify-center opacity-60">
                            <span className="text-[9px] font-black text-zinc-400">DRIVER</span>
                            <span className="text-xs font-black text-zinc-500 mt-1">👨‍✈️ Captain</span>
                          </div>
                          
                          <div className="col-span-1" /> {/* Center Walkway */}

                          {/* Render all passenger seat positions from 1 to C */}
                          {(() => {
                            const seatElements = [];
                            for (let i = 1; i <= totalSeats; i++) {
                              const booking = bookingsForTrip.find(b => b.seatNumber === i);
                              const passenger = booking ? dbStudents.find(s => s.id === booking.studentId) : null;
                                  ...
                                  {booking && passenger ? (
                                    ...
                                    <div className="bg-zinc-900 border border-zinc-800 text-white p-2.5 rounded-xl flex flex-col justify-between h-[82px] shadow-sm relative dark:bg-zinc-950">
                                      <div className="flex justify-between items-start">
                                        ...
                                        <button
                                          type="button"
                                          onClick={() => handleUnassignSeat(booking.id)}
                                      ...
                                      <p className="text-[10px] font-black truncate mt-1.5" title={passenger.studentName}>
                                        {passenger.studentName.split(' ').slice(1).join(' ') || passenger.studentName}
                                      </p>
                                    ...
                                    >
                                      ...
                                      <span className="text-[10px] font-black tracking-wider block mt-1.5 uppercase">FREE</span>
                                      <span className="text-[8px] text-zinc-400 font-bold">Book Seat</span>
                                    </button>
                                  )}
                                </div>
                              );
                    ...
                    <div>
                      <h4 className="text-xs font-black text-zinc-800 dark:text-white mb-3 uppercase tracking-wider">
                        Trip Passenger Manifest ({bookingsForTrip.length} Students)
                      </h4>
                      <div className="space-y-2 max-h-[220px] overflow-y-auto">
                        {bookingsForTrip.map(b => {
                          const s = dbStudents.find(stud => stud.id === b.studentId);
                          if (!s) return null;
                          return (
                            <div key={b.id} className="border border-slate-50 bg-slate-50/20 p-2.5 rounded-xl flex items-center justify-between text-xs dark:border-zinc-800 dark:bg-zinc-950/40">
                              <div className="flex items-center gap-2.5">
                                <span className="font-bold text-[11px] bg-zinc-800 text-white px-2 py-0.5 rounded dark:bg-zinc-800">
                                  Seat #{b.seatNumber || '?'}
                                </span>
                                <div>
                                  <p className="font-bold text-zinc-800 dark:text-zinc-200">{s.studentName}</p>
                                  <p className="text-[10px] text-zinc-400 font-medium">Student ID: {s.studentNumber} • Sec {s.section}</p>
                                </div>
                              </div>
                              <button
                                type="button"
                        ...
                        {bookingsForTrip.length === 0 && (
                          <p className="text-xs text-zinc-400 text-center py-6 italic">No passengers assigned to this trip yet. Click an empty seat above to assign students.</p>
                        )}
                      </div>
                    </div>
                  </div>
                ...
                <div className="flex flex-col items-center justify-center py-24 border border-dashed border-slate-200 rounded-2xl bg-white dark:bg-zinc-900 dark:border-zinc-800">
                  ...
                  <p className="text-[11px] text-zinc-400 mt-1 max-w-sm text-center">
                    Please select a scheduled trip from the commutes panel on the left to review passenger assignments and manage seats.
                  </p>
                </div>
              )}
            </div>
              ...
              <div className="bg-white rounded-2xl w-full max-w-md p-6 shadow-xl border border-slate-100 dark:bg-zinc-900 dark:border-zinc-800 space-y-4">
                ...
                <div className="space-y-3">
                  ...
                  <div className="max-h-[250px] overflow-y-auto space-y-1.5 pr-1">
                    {(() => {
                      const trip = dbTransportSchedules.find(t => t.id === activeTripId);
                      const currentBookings = dbTransportAssignments.filter(b => b.scheduleId === activeTripId && b.status === 'active');
                      
                      // List of students that aren't already booked on this trip's other seats
                      const candidateStudents = dbStudents.filter(s => {
                        const matchesSearch = s.studentName.toLowerCase().includes(studentAssignSearch.toLowerCase()) || 
                                              s.studentNumber.toLowerCase().includes(studentAssignSearch.toLowerCase());
                        const isAlreadyBookedOnAnotherSeat = currentBookings.some(b => b.studentId === s.id && b.seatNumber !== assigningSeatNumber);
                      ...
                      return (
                        <>
                          ...
                          {candidateStudents.length === 0 && (
                            <p className="text-xs text-zinc-400 text-center py-6 italic">No matching eligible students found.</p>
                          )}
                        </>
                      );
                    })()}
        ...
        <div className="space-y-6">
          ...
          {utilitiesSubTab === 'record' && (
            <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
              {/* Left Column: Form */}
              <form onSubmit={handleGenerateBills} className="lg:col-span-8 space-y-6">
                <div className="bg-white border border-slate-100 p-6 rounded-2xl dark:bg-zinc-900 dark:border-zinc-800 space-y-4">
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
                    <div>
                      <label className="block text-[10px] font-black text-zinc-400 uppercase mb-1">Target Room</label>
                      <select
                      ...
                      >
                        ...
                        {dbRooms.map(r => {
                          const occupants = dbStudents.filter(s => s.roomId === r.id);
                          return (
                            <option key={r.id} value={r.id}>
                    ...
                    <div>
                      <label className="block text-[10px] font-black text-zinc-400 uppercase mb-1">Billing Month</label>
                      <select
                        value={selectedMonth}
                    ...
                    <div>
                      <label className="block text-[10px] font-black text-zinc-400 uppercase mb-1">Billing Year</label>
                      <select
                        value={selectedYear}
                ...
                <div className="bg-white border border-slate-100 p-6 rounded-2xl dark:bg-zinc-900 dark:border-zinc-800 space-y-4">
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
                    ...
                    <div className="p-4 rounded-xl bg-blue-50/10 border border-blue-100/50 space-y-3">
                      ...
                      <div className="grid grid-cols-2 gap-2">
                        <div>
                          <label className="block text-[9px] font-black text-zinc-400 uppercase">Previous Meter</label>
                          <input
                            type="number"
                            min="0"
                            value={prevWaterMeter}
                            onChange={(e) => setPrevWaterMeter(parseInt(e.target.value) || 0)}
                        ...
                        <div>
                          <label className="block text-[9px] font-black text-zinc-400 uppercase">Current Meter</label>
                          <input
                            type="number"
                            min="0"
                            value={currWaterMeter}
                            onChange={(e) => setCurrWaterMeter(parseInt(e.target.value) || 0)}
                            disabled={!!waterAmountOverride}
                            className="w-full rounded-lg border border-slate-100 bg-white px-2.5 py-1.5 text-xs outline-hidden dark:border-zinc-800 dark:bg-zinc-900"
                          />
                        </div>
                      ...
                      <div className="grid grid-cols-2 gap-2 pt-1">
                        <div>
                          <label className="block text-[9px] font-black text-zinc-400 uppercase">Rate (฿/unit)</label>
                          <input
                            type="number"
                            min="1"
                            value={waterRate}
                        ...
                        <div>
                          <label className="block text-[9px] font-black text-zinc-400 uppercase">Direct Cost Override (฿)</label>
                          <input
                            type="text"
                            placeholder="Direct input"
                            value={waterAmountOverride}
                            onChange={(e) => setWaterAmountOverride(e.target.value)}
                      ...
                      <div className="grid grid-cols-2 gap-2">
                        <div>
                          ...
                          <input
                            ...
                            onChange={(e) => setPrevElectricMeter(parseInt(e.target.value) || 0)}
                        ...
                        <div>
                          ...
                          <input
                            ...
                            onChange={(e) => setCurrElectricMeter(parseInt(e.target.value) || 0)}
                            disabled={!!electricAmountOverride}
                            className="w-full rounded-lg border border-slate-100 bg-white px-2.5 py-1.5 text-xs outline-hidden dark:border-zinc-800 dark:bg-zinc-900"
                          />
                        </div>
                      ...
                      <div className="grid grid-cols-2 gap-2 pt-1">
                        ...
                        <div>
                          ...
                          <input
                            ...
                            onChange={(e) => setElectricAmountOverride(e.target.value)}
                            className="w-full rounded-lg border border-slate-100 bg-white px-2.5 py-1.5 text-xs outline-hidden dark:border-zinc-800 dark:bg-zinc-900 font-bold"
                          />
                        </div>
                      </div>
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    <div>
                      <label className="block text-[10px] font-black text-zinc-400 uppercase mb-1">Other Surcharges (฿)</label>
                      <input
                        type="number"
                        min="0"
                        value={otherCharges}
                    ...
                    <div>
                      <label className="block text-[10px] font-black text-zinc-400 uppercase mb-1">Billing Notes / Remarks</label>
                      <input
                        type="text"
                ...
                {selectedRoomId ? (() => {
                  const occupants = dbStudents.filter(s => s.roomId === selectedRoomId);
                  const calcWaterUnits = Math.max(0, currWaterMeter - prevWaterMeter);
                  const waterCost = waterAmountOverride ? parseFloat(waterAmountOverride) || 0 : calcWaterUnits * waterRate;

                  const calcElectricUnits = Math.max(0, currElectricMeter - prevElectricMeter);
                  const electricCost = electricAmountOverride ? parseFloat(electricAmountOverride) || 0 : calcElectricUnits * electricRate;

                  const totalRoomBill = waterCost + electricCost + otherCharges;
                  const baseSplitAmount = occupants.length > 0 ? totalRoomBill / occupants.length : 0;
                    ...
                    <div className="bg-white border border-slate-100 p-6 rounded-2xl dark:bg-zinc-900 dark:bg-zinc-800 space-y-4">
                      ...
                      {occupants.length === 0 ? (
                        <div className="text-center py-6 border border-dashed rounded-xl">
                          <p className="text-xs text-zinc-500 font-bold">No active roommates assigned to this room yet.</p>
                          <button
                            type="button"
                            onClick={() => setUtilitiesSubTab('rooms')}
                        ...
                        <div className="space-y-4">
                          <div className="flex justify-between text-xs font-bold text-zinc-500 bg-slate-50 dark:bg-zinc-950 p-3 rounded-xl border">
                            <span>Base Room Bill: ฿{totalRoomBill.toFixed(2)}</span>
                            <span>Auto Split Cost: <span className="text-red-600 font-black">฿{baseSplitAmount.toFixed(2)} / student</span></span>
                          </div>

                          <div className="space-y-3">
                            {occupants.map(student => {
                              const studentAdj = adjustments[student.id] || { adjustment: 0, note: '' };
                              const finalAmount = Math.max(0, baseSplitAmount + studentAdj.adjustment);

                              return (
                                <div key={student.id} className="p-3 border border-slate-100 dark:border-zinc-850 rounded-xl flex flex-col md:flex-row md:items-center justify-between gap-4">
                                  <div className="flex items-center gap-3">
                                    ...
                                    <div>
                                      <p className="text-xs font-black text-zinc-900 dark:text-white">{student.studentName}</p>
                                      <p className="text-[10px] text-zinc-400 font-bold">ID: {student.studentNumber} • Section: {student.section}</p>
                                    </div>
                                  </div>
                                  ...
                                  <div className="flex flex-wrap items-center gap-3">
                                    ...
                                    <div className="flex items-center gap-1.5">
                                      <span className="text-[10px] font-bold text-zinc-400">Adj (฿):</span>
                                      <input
                                        type="number"
                                        placeholder="0"
                                        value={studentAdj.adjustment === 0 ? '' : studentAdj.adjustment}
                                        onChange={(e) => {
                                          const val = parseFloat(e.target.value) || 0;
                                          setAdjustments(prev => ({
                                            ...prev,
                                            [student.id]: { ...studentAdj, adjustment: val }
                                          }));
                                        }}
                                    ...
                                    <div>
                                      <input
                                        ...
                                        onChange={(e) => {
                                          setAdjustments(prev => ({
                                            ...prev,
                                            [student.id]: { ...studentAdj, note: e.target.value }
                                          }));
                                        }}
                                    ...
                                    <div className="text-right min-w-[70px]">
                                      <p className="text-[9px] text-zinc-400 font-bold uppercase">Final Cost</p>
                                      <p className="text-xs font-black text-zinc-900 dark:text-white">฿{finalAmount.toFixed(2)}</p>
                                    </div>
                                  </div>
                                </div>
                              );
                          ...
                          <div className="pt-4 border-t flex justify-end">
                            ...
                            >
                              <Check className="h-4 w-4" /> Save & Publish Room Bills
                            </button>
                          </div>
                        </div>
                      )}
                  ...
                  <div className="bg-white border border-slate-100 p-8 rounded-2xl dark:bg-zinc-900 dark:border-zinc-800 text-center text-zinc-400">
                    <p className="text-xs font-bold">Please select a target room above to proceed with bill split calculation.</p>
                  </div>
                )}
              </form>
              ...
              <div className="lg:col-span-4 space-y-6">
                <div className="bg-white border border-slate-100 p-5 rounded-2xl dark:bg-zinc-900 dark:border-zinc-800 space-y-4">
                  ...
                  <p className="text-xs text-zinc-500 dark:text-zinc-400 leading-relaxed">
                    Electricity and water meter values are read monthly by dormitory supervisors. The system automatically divides the total costs of the room equally based on occupancy.
                  </p>
                  <div className="p-3.5 bg-slate-50 dark:bg-zinc-950 rounded-xl border text-[11px] space-y-2 text-zinc-600 dark:text-zinc-400">
                    <p className="font-bold flex items-center gap-1.5">
                      <span className="h-2 w-2 rounded-full bg-blue-500" /> Standard Rates:
                    </p>
                    <ul className="list-disc list-inside pl-2 space-y-1">
                      <li>Water: ฿10 per cubic meter unit</li>
                      <li>Electric: ฿5 per kW/h unit</li>
                      <li>Adjustment overrides will reflect in students ledger.</li>
                    </ul>
                  </div>
                </div>
              </div>
          ...
          {utilitiesSubTab === 'rooms' && (
            <div className="bg-white border border-slate-100 p-6 rounded-2xl dark:bg-zinc-900 dark:border-zinc-800 space-y-6">
              <div>
                <h3 className="text-base font-black text-zinc-900 dark:text-white mb-1">Dormitory Rooms Assignment Manager</h3>
                <p className="text-xs text-zinc-500">Quickly coordinate student occupancy inside rooms. Click remove to unassign, or select an unassigned student to assign them to a room.</p>
              </div>

              {/* Grid of rooms */}
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                {dbRooms.map(room => {
                  const occupants = dbStudents.filter(s => s.roomId === room.id);
                  const unassigned = dbStudents.filter(s => !s.roomId && s.status === 'active');

                  return (
                    <div key={room.id} className="border border-slate-100 rounded-2xl p-5 dark:border-zinc-800 bg-slate-50/10 space-y-4 shadow-sm flex flex-col justify-between">
                      <div>
                        <div className="flex justify-between items-start">
                          <div>
                            <h4 className="text-sm font-black text-zinc-800 dark:text-white flex items-center gap-1.5">
                              <Home className="h-4 w-4 text-zinc-400" /> Room {room.roomNumber}
                            </h4>
                            <p className="text-[10px] text-zinc-400 font-semibold mt-0.5">Floor {room.floorId || 'Default'} • Cap: {room.capacity || 4} Beds</p>
                          </div>
                          <span className={`text-[9px] font-black uppercase px-2 py-0.5 rounded-full ${occupants.length >= (room.capacity || 4) ? 'bg-amber-100 text-amber-700' : 'bg-emerald-50 text-emerald-700'}`}>
                        ...
                        <div className="mt-4 space-y-1.5">
                          <span className="text-[9px] font-black uppercase tracking-wider text-zinc-400">Current Occupants</span>
                          {occupants.map(student => (
                            <div key={student.id} className="bg-white dark:bg-zinc-950 p-2 rounded-xl border border-slate-100 dark:border-zinc-850 flex justify-between items-center">
                          ...
                          {occupants.length === 0 && (
                            <p className="text-[11px] text-zinc-400 italic py-2">No students assigned to this room yet.</p>
                          )}
                        </div>
                      </div>
                      ...
                      {occupants.length < (room.capacity || 4) && (
                        <div className="mt-4 pt-3 border-t border-slate-100 dark:border-zinc-850 flex gap-1.5 items-center">
                          <select
                            ...
                            onChange={(e) => setAssignRoomSelectedStudentId(e.target.value)}
                          ...
                          >
                            ...
                            {unassigned.map(s => (
                              <option key={s.id} value={s.id}>{s.studentName} ({s.studentNumber})</option>
                            ))}
                          </select>
                          <button
                            onClick={() => {
                              if (!assignRoomSelectedStudentId) {
                                triggerToast('Please select a student', 'error');
                                return;
                              }
                              handleAssignRoom(assignRoomSelectedStudentId, room.id);
          ...
          {utilitiesSubTab === 'ledger' && (
            <div className="space-y-6">
              ...
              {verifyingPayment && (() => {
                const correspondingBill = dbBills.find(b => b.billId === verifyingPayment.billId);
                const studentProfile = dbStudents.find(s => s.id === verifyingPayment.tenantId);
                    ...
                    <div className="bg-white dark:bg-zinc-900 border dark:border-zinc-850 rounded-2xl shadow-xl w-full max-w-3xl overflow-hidden animate-in fade-in zoom-in-95 duration-150">
                      <div className="bg-gradient-to-r from-red-600 to-rose-700 px-6 py-4 text-white flex justify-between items-center">
                        <h4 className="text-sm font-black uppercase tracking-wider flex items-center gap-1.5">
                          <FileText className="h-5 w-5" /> Bank Payment Slip Verification
                        </h4>
                        <button
                          onClick={() => {
                            setVerifyingPayment(null);
                      ...
                      <div className="p-6 grid grid-cols-1 md:grid-cols-2 gap-6">
                        ...
                        <div className="space-y-2 border-r pr-6 dark:border-zinc-800">
                          <span className="text-[10px] font-black text-zinc-400 uppercase tracking-widest block">Student Upload Preview</span>
                          {verifyingPayment.slipUrl ? (
                            ...
                            <div className="border border-dashed rounded-xl h-72 flex flex-col items-center justify-center text-zinc-400">
                              ...
                              <p className="text-xs font-bold mt-2">No slip preview image found.</p>
                            </div>
                          )}
                          <div className="text-[11px] text-zinc-400 flex justify-between pt-1">
                            <span>Uploaded: {new Date(verifyingPayment.uploadTime).toLocaleString()}</span>
                            <span>Transferred: {verifyingPayment.transferDate}</span>
                          </div>
                        </div>
                        ...
                        <div className="space-y-4 flex flex-col justify-between">
                          <div className="space-y-3">
                            <span className="text-[10px] font-black text-zinc-400 uppercase tracking-widest block">Bill & Payer Details</span>
                            ...
                            <div className="bg-slate-50 dark:bg-zinc-950 p-3.5 rounded-xl border text-xs space-y-1.5 text-zinc-800 dark:text-zinc-200">
                              <p className="font-bold">
                                Student: <span className="font-black text-zinc-950 dark:text-white">{studentProfile?.studentName || verifyingPayment.tenantId}</span>
                              </p>
                              <p>Student ID: {studentProfile?.studentNumber} • Section: {studentProfile?.section}</p>
                              <p className="border-t pt-1.5 mt-1 text-red-600 dark:text-red-400 font-black text-[11px]">
                                Room {correspondingBill?.roomId || verifyingPayment.roomId} Bill • {correspondingBill?.month} {correspondingBill?.year}
                              </p>
                              <div className="grid grid-cols-2 gap-2 text-[10px] text-zinc-500 pt-1">
                                <p>Water Split: ฿{correspondingBill?.waterAmount}</p>
                                <p>Electric Split: ฿{correspondingBill?.electricAmount}</p>
                              </div>
                            </div>

                            <div className="text-center p-3 bg-red-50 dark:bg-red-950/20 rounded-xl border border-red-100">
                              <p className="text-[10px] font-black text-red-600 uppercase">Paid Transfer Amount</p>
                              <p className="text-2xl font-black text-red-700">฿{verifyingPayment.amount.toFixed(2)}</p>
                            </div>

                            <div className="space-y-1.5">
                              <label className="block text-[10px] font-black text-zinc-400 uppercase">Rejection Reason Remark</label>
                              <input
                                type="text"
                          ...
                          <div className="grid grid-cols-2 gap-3 pt-4 border-t">
                            <button
                              onClick={() => handleRejectSlip(verifyingPayment.paymentId)}
                            ...
                            <button
                              onClick={() => handleApproveSlip(verifyingPayment.paymentId)}
                ...
                <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-4">
                  ...
                  <div className="flex flex-wrap items-center gap-2">
                    ...
                    >
                      ...
                      {['มิถุนายน', 'กรกฎาคม', 'สิงหาคม', 'กันยายน', 'ตุลาคม', 'พฤศจิกายน', 'ธันวาคม'].map(m => (
                        <option key={m} value={m}>{m}</option>
                      ))}
                    </select>
                    <select
                ...
                <div className="overflow-x-auto">
                  <table className="w-full text-left border-collapse text-xs">
                    ...
                    <tbody className="divide-y divide-slate-50/50 dark:divide-zinc-800/40">
                      {dbBills
                        .filter(b => !ledgerRoomId || b.roomId === ledgerRoomId)
                        .filter(b => !ledgerMonth || b.month === ledgerMonth)
                        .filter(b => !ledgerStatus || b.status === ledgerStatus)
                        .map(b => {
                          const studentObj = dbStudents.find(s => s.id === b.tenantId);
                          const matchingPayment = dbPayments.find(p => p.billId === b.billId && p.status === 'Pending');

                          return (
                            <tr key={b.billId} className="hover:bg-slate-50/30 dark:hover:bg-zinc-900/30">
                              <td className="py-3 pl-3 font-black text-zinc-900 dark:text-white">
                                Room {dbRooms.find(r => r.id === b.roomId)?.roomNumber || b.roomId}
                              </td>
                              <td className="py-3 font-semibold text-zinc-500">
                                {b.month} {b.year}
                              </td>
                              <td className="py-3">
                                <p className="font-bold text-zinc-800 dark:text-zinc-200">{studentObj?.studentName || b.tenantId}</p>
                                <p className="text-[9px] text-zinc-400 font-semibold">ID: {studentObj?.studentNumber}</p>
                              </td>
                              ...
                              <td className="py-3 pr-3 text-right">
                                <div className="inline-flex items-center gap-1.5">
                                  {b.status === 'Pending' && matchingPayment && (
                                    <button
                                      onClick={() => setVerifyingPayment(matchingPayment)}
                                  ...
                                  <button
                                    onClick={() => handleDeleteTeacherBill(b.billId)}
                      ...
                      {dbBills.length === 0 && (
                        <tr>
                          <td colSpan={8} className="py-8 text-center text-zinc-400 italic font-semibold">
                            No student bills have been recorded or generated yet.
                          </td>
                        </tr>
                      )}
                    </tbody>
      ...
      {activeTab === 'documents' && (
        <div className="space-y-6">
          <div className="rounded-2xl bg-white border border-slate-100 p-6 dark:bg-zinc-900 dark:border-zinc-800">
            <h3 className="text-lg font-black text-zinc-900 dark:text-white mb-2">Academic Placement Guidelines</h3>
            <p className="text-xs text-zinc-500 dark:text-zinc-400 mb-6">Distribute evaluation logbooks, consent forms, and guidelines manuals to students.</p>
            ...
            <form onSubmit={handleAddDoc} className="flex gap-3 mb-6">
              ...
              />
              <button type="submit" className="flex items-center gap-1 bg-red-600 px-4 py-2 text-xs font-black text-white rounded-xl shadow-xs hover:bg-red-500 transition cursor-pointer">
            ...
            <div className="overflow-x-auto">
              <table className="w-full text-left border-collapse text-xs">
                ...
                <tbody className="divide-y divide-slate-50/50 dark:divide-zinc-800/40">
                  {documents.map(d => (
                    <tr key={d.id} className="hover:bg-slate-50/30 dark:hover:bg-zinc-900/30">
                      ...
                      <td className="py-3 pr-3 text-right text-zinc-400">{d.date}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
      ...
      {activeTab === 'announcements' && (
        <div className="space-y-6">
          <div className="rounded-2xl bg-white border border-slate-100 p-6 dark:bg-zinc-900 dark:border-zinc-800">
            <h3 className="text-lg font-black text-zinc-900 dark:text-white mb-2">Bulletin Announcement Board</h3>
            <p className="text-xs text-zinc-500 dark:text-zinc-400 mb-6">Post updates and critical schedule news directly onto students dashboards.</p>

            <form onSubmit={handleAddAnn} className="space-y-4 mb-8 p-4 bg-slate-50/50 dark:bg-zinc-950/30 rounded-2xl border border-slate-100 dark:border-zinc-800">
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                <div className="md:col-span-2">
                  <label className="block text-[10px] font-black text-zinc-400 uppercase mb-1">Headline Title</label>
                  <input 
                    type="text" 
                ...
                <div>
                  <label className="block text-[10px] font-black text-zinc-400 uppercase mb-1">Urgency Priority</label>
                  <select 
                    value={annPriority} 
              ...
              <div>
                <label className="block text-[10px] font-black text-zinc-400 uppercase mb-1">Notice Content</label>
                <textarea 
                  rows={3}
              ...
              <button type="submit" className="flex items-center gap-1.5 bg-red-600 px-4 py-2.5 text-xs font-black text-white rounded-xl shadow-xs hover:bg-red-500 transition cursor-pointer">
            ...
            <div className="space-y-4">
              {announcements.map(a => (
                <div key={a.id} className="rounded-xl border border-slate-100 p-5 dark:border-zinc-800 bg-slate-50/20 dark:bg-zinc-900/10">
                  <div className="flex items-center justify-between">
                    <div className="flex items-center gap-2">
                      <span className={`text-[10px] font-black uppercase px-2 py-0.5 rounded-md ${a.priority === 'Urgent' ? 'bg-red-100 text-red-700' : 'bg-slate-100 text-slate-700 dark:bg-zinc-800 dark:text-zinc-300'}`}>
                        {a.priority}
                      </span>
                      <span className="text-[11px] text-zinc-400 font-bold">Posted by {a.author}</span>
                    </div>
                    <span className="text-xs text-zinc-400 font-bold">{a.date}</span>
                  </div>
                  <h4 className="text-sm font-black text-zinc-900 dark:text-white mt-3">{a.title}</h4>
                  <p className="text-xs text-zinc-600 dark:text-zinc-400 mt-2 leading-relaxed">{a.content}</p>
                </div>
              ))}
            </div>
          </div>
      ...
      {activeTab === 'reports' && (
        <div className="space-y-6">
          <div className="rounded-2xl bg-white border border-slate-100 p-6 dark:bg-zinc-900 dark:border-zinc-800">
            <h3 className="text-lg font-black text-zinc-900 dark:text-white mb-2">Srisavarindhira Operational Reports</h3>
            <p className="text-xs text-zinc-500 dark:text-zinc-400 mb-6">Download clinical logs, dormitory checklists, or utility statements as high-quality CSV/Excel formats.</p>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div className="rounded-xl border border-slate-100 p-4 dark:border-zinc-800 bg-slate-50/30 dark:bg-zinc-900/20 flex items-center justify-between">
                <div>
                  <h4 className="text-xs font-black text-zinc-800 dark:text-zinc-200 uppercase tracking-wider">Clinical Cohort Placement.csv</h4>
                  <p className="text-[11px] text-zinc-400 mt-1">Export groups, sites, and supervising instructors logs.</p>
                </div>
                <button onClick={() => triggerToast('CSV exported')} className="rounded-xl bg-zinc-900 hover:bg-zinc-800 text-white p-2.5 cursor-pointer dark:bg-white dark:text-zinc-950 dark:hover:bg-zinc-100">
              ...
              <div className="rounded-xl border border-slate-100 p-4 dark:border-zinc-800 bg-slate-50/30 dark:bg-zinc-900/20 flex items-center justify-between">
                <div>
                  <h4 className="text-xs font-black text-zinc-800 dark:text-zinc-200 uppercase tracking-wider">Lodging Rooms & Beds Roster.xlsx</h4>
                  <p className="text-[11px] text-zinc-400 mt-1">Export student dormitory assignments and vacancies.</p>
                </div>
                <button onClick={() => triggerToast('Excel exported')} className="rounded-xl bg-zinc-900 hover:bg-zinc-800 text-white p-2.5 cursor-pointer dark:bg-white dark:text-zinc-950 dark:hover:bg-zinc-100">
                  <FileDown className="h-4.5 w-4.5" />
                </button>
              </div>
            </div>
      ...
      {activeTab === 'settings' && (
        <div className="space-y-6">
          <div className="rounded-2xl bg-white border border-slate-100 p-6 dark:bg-zinc-900 dark:border-zinc-800">
            <h3 className="text-lg font-black text-zinc-900 dark:text-white mb-2">System Configurations</h3>
            <p className="text-xs text-zinc-500 dark:text-zinc-400 mb-6">Manage system defaults and placement parameters.</p>

            <div className="space-y-6">
              <div className="flex items-center justify-between py-3 border-b dark:border-zinc-800">
                <div>
                  <h4 className="text-sm font-bold text-zinc-800 dark:text-zinc-200">Active Semester Term</h4>
                  <p className="text-xs text-zinc-400">Current calendar cycle for new placements</p>
                </div>
                <select 
              ...
              <div className="flex items-center justify-between py-3">
                <div>
                  <h4 className="text-sm font-bold text-zinc-800 dark:text-zinc-200">Automatic Lodging Allocation</h4>
                  <p className="text-xs text-zinc-400">Require automated system checks during student check-ins</p>
                </div>
                <button 
